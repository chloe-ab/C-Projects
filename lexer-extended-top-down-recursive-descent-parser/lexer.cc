...
void SymbolTable::addAssignment(string lhs, string rhs) {
	assignments.push_back(std::make_pair(findVariable(lhs),
		findVariable(rhs)));
}
...
void Parser::print() {
	...
	for (i = table.assignments.begin(); i != table.assignments.end(); ++i) {
		cout << i->first.declaration << ((i->first.declaration != "::") ?
			"." : "") << i->first.variable << " = " << i->second.declaration << ((i -> second.declaration != "::") ? "." : "") << i->second.variable << endl;
	}
}
